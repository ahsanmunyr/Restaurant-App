    ...
    >
      ...
      <Tab.Screen
        ...
        options={{
          ...
          tabBarLabel: ({ color, size, focused }) => (
            focused ?
              <Text style={{ color: color, fontFamily: 'Overpass-Bold', fontSize: 12, bottom: 5 }}>History</Text> : null
          ),
          tabBarLabelStyle: { bottom: 5, color: 'white' },
          tabBarIcon: ({ color, size }) => (
            <MaterialIcons name="history" style={{}} size={size} color={color} />
          ),
        }}
      />
      <Tab.Screen
        ...
        options={{
          ...
          tabBarLabel: ({ color, size, focused }) => (
            focused ?
              <Text style={{ color: color, fontFamily: 'Overpass-Bold', fontSize: 12, bottom: 5 }}>Search</Text> : null
          ),
          tabBarLabelStyle: { bottom: 5, color: 'white' },
          tabBarIcon: ({ color, size }) => (
            <View style={{ position: 'absolute', backgroundColor: '#f54749', bottom: 15, height: 65, flex: 1, justifyContent: 'center', width: 65, alignItems: 'center', borderRadius: 50 }}>
      ...
      <Tab.Screen
        ...
        options={{
          ...
          tabBarLabel: ({ color, size, focused }) => (
            focused ?
              <Text style={{ color: color, fontFamily: 'Overpass-Bold', fontSize: 12, bottom: 5 }}>Cart</Text> : null
          ),
          tabBarLabelStyle: { bottom: 5, color: 'white' },
          tabBarIcon: ({ color, size }) => (
            <Ionicons name="cart-outline" style={{}} size={size} color={color} />
          ),
        }}
      />
      <Tab.Screen
        ...
        options={{
          ...
          tabBarLabel: ({ color, size, focused }) => (
            focused ?
              <Text style={{ color: color, fontFamily: 'Overpass-Bold', fontSize: 12, bottom: 5 }}>Setting</Text> : null
          ),
          tabBarLabelStyle: { bottom: 5, color: 'white' },
          tabBarIcon: ({ color, size }) => (
            <Ionicons name="settings-outline" style={{}} size={size} color={color} />
          ),
        }}
      />
    </Tab.Navigator>
...
function MainAppScreens({ UserLatLong, UserLocation,
  getRestaurant, getDeals, userAddToCart, orderDetails,
  acceptOrderDetails, placeOrderStatus, orderAccept,
  userDetailsSave, navigationApp, orderPlaceDataClear,
  firebaseCoordsRider, orderDataClear, firebaseMessageData }) {
  ...
  useEffect(() => {
    ...
    if (placeOrderStatus.order) {
      // alert("SAD")
      onChangeStatus(placeOrderStatus.order)
      orderDetails(placeOrderStatus.data.order_id)
    }
    ...
    return (
      <>
        <STACK.Navigator initialRouteName={initialRoute}>
          ...
          <STACK.Screen name="Map" options={{ headerShown: false, }} component={MapStack} />
          <STACK.Screen name="OrderStack" options={{ headerShown: false, }} component={OrderStack} />
          <STACK.Screen name="ProcessStack" options={{ headerShown: false, }} component={ProcessStack} />
        </STACK.Navigator>
        {status ? (
          <View
            style={{
...
function mapStateToProps({ userAddToCart, orderAccept, placeOrderStatus, navigationApp }) {
